    ...
    def Es(self, k, d=0):
        ...
        if d == 0:
            ...
            raise NotImplementedError(
                "Only d=0, 1, or 2 supported. (got d={})".format(d)
            )
...
class SOC2(GPEBase):
    ...
    def set_initial_data(self):
        ...
        ka = self.k0 + self.k_R
        kb = self.k0 - self.k_R
        assert np.allclose(0, [abs(ka - kx_).min(), abs(kb - kx_).min()])
        ...
        self.data = (
            np.ones(self.Nxy, dtype=complex)[None, ...] * np.sqrt(n0) * psi_ab * phase
        )
        self._N = self.get_density().sum()
    ...
    def apply_expV(self, dt, factor=1.0, density=None):
        ...
        Vab = self._Vab
        _tmp = self._phase * dt * factor * np.array([[Va, Vab], [Vab.conj(), Vb]])
...
class SOC1(GPEBase):
    ...
    layout = w.VBox(
        [
            w.FloatSlider(name="soc_w", min=-3, max=3, step=0.1, description=r"w"),
            w.FloatSlider(name="soc_d", min=-2, max=2, step=0.1, description=r"d"),
            GPEBase.layout,
        ]
    )
    ...
    def init(self):
        ...
        self.dispersion = Dispersion(w=self.soc_w, d=self.soc_d, k0=k0, E0=E0)
        self.k0 = 0.0
...
class SuperSolid2(SOC2):
    ...
    layout = w.VBox(
        [
            w.FloatSlider(
                name="lattice_k_k_R", min=-3, max=3, step=0.1, description=r"k_L/k_R"
            ),
            w.FloatSlider(
                name="lattice_V0_mu", min=-2, max=2, step=0.1, description=r"V_L"
            ),
            w.FloatSlider(
                name="lattice_x0", min=-5, max=5, step=0.1, description=r"x_0"
            ),
            SOC2.layout,
        ]
    )
